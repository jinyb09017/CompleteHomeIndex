...
public class ViewUtil {
	...
	public static class OnGlobalLayoutListenerQueueExecutor implements
	 ...
	 */
	public static void addGlobalLayoutListenerOnce(final View view,
		...
		if (onGlobalLayoutListener != null) {
			if (onGlobalLayoutListener != null) {
				if (isCachedViewTreeObserver(view)) {
					...
					sViewTreeObserverListenerMap.get(view).add(
					...
					final OnGlobalLayoutListenerQueueExecutor tempListener = new OnGlobalLayoutListenerQueueExecutor() {
						...
						public void onGlobalLayout() {
							Log.d(TAG,
									"OnGlobalLayoutListenerQueueExecutor called ");
							...
							if (isCachedViewTreeObserver(view)) {
								// ToDo thread safe
								try {
									for (ViewTreeObserver.OnGlobalLayoutListener entry : sViewTreeObserverListenerMap
											.get(view)) {
										if (entry != null) {
	...
	@TargetApi(16)
	static void removeOnGlobalLayoutListener(View view,
			OnGlobalLayoutListenerQueueExecutor onGlobalLayoutListener) {
		view.getViewTreeObserver().removeGlobalOnLayoutListener(
				onGlobalLayoutListener);
	}

	static void removeOnGlobalLayoutListener16old(View view,
			OnGlobalLayoutListenerQueueExecutor onGlobalLayoutListener) {
		view.getViewTreeObserver().removeGlobalOnLayoutListener(
				onGlobalLayoutListener);
	}
